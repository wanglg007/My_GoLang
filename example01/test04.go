...
func main(){
	// Go提供了几种打印格式，用来格式化一般的Go值，例如下面的%v打印了一个point结构体的对象的值
	p := point{1,2}
	fmt.Printf("%v\n",p)
	...
	// 当输出数字的时候，经常需要去控制输出的宽度和精度。可以使用一个位于%后面的数字来控制输出的宽度，默认
	// 情况下输出是右对齐的，左边加上空格
	fmt.Printf("|%6d|%6d|\n", 12, 345)
	...
	// `Printf`函数的输出是输出到命令行`os.Stdout`的，可以用`Sprintf`来将格式化后的字符串赋值给一个变量
	s := fmt.Sprintf("a %s", "string")
	fmt.Println(s)

	// 可以使用`Fprintf`来将格式化后的值输出到`io.Writers`
