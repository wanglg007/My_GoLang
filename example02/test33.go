...
//(33)集合功能
//需要程序去处理一些集合数据，比如选出所有符合条件的数据或者使用一个自定义函数将一个集合元素拷贝到另外一个集合。
...
// 如果使用vs中的任何一个字符串作为函数f的参数可以让f返回true，那么返回true，否则false
...
// 如果分别使用vs中所有的字符串作为f的参数都能让f返回true，那么返回true，否则返回false
...
// 返回一个新的字符串切片，切片的元素为vs中所有能够让函数f返回true的元素
...
// 返回一个bool类型切片，切片的元素为vs中所有字符串作为f函数参数所返回的结果
